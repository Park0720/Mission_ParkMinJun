...
- [ ] `AppConfig`의 변수명 변경
- [ ] `calDiffTime` 함수 Ut로 옮겨서 중복 제거하기
- [ ] `calDiffTime` 함수에 로컬 변수 생성해서 처리 시간 빠르게 하기
- [ ] 설정파일에서 시간 단위 설정을 초 단위 설정으로 바꾸기
- [ ] `readDate` 변경 시에 알림페이지 방문할 때마다 업데이트 되던 것 최초 1회만 업데이트되게 변경